let nDates = 0;
  ...
  var testData = "APM MR O1 SON5,Monday, 16:00:00 ,,Phy AP O2 SON1,Monday, 04:00:00,Sun Aug 24 2025,Phy AP P4 SON10,Wednesday,14:00:00,,STN MR O5 SOF5,Wednesday,04:00:00,";
  const testDiv = document.getElementById('test');
  const batchSummary = document.getElementById('batchSummary');
  const batchDetails = document.getElementById('batchDetails');
  const scheduleSetting = document.getElementById('scheduleSetting');
  const dateList = document.getElementById('dateList');
  const loginUrl = "https://twc.physer.workers.dev";
  var studentId = '';


function loadTimezone() {
  createTimezoneSelector('testTimezoneInput','timezoneListSh','displayTimeSh','utcValue');
}
...
async function getBatches() {
  ...
  var loginCred = new FormData(document.getElementById('loginForm'));
  document.getElementById('loginForm').reset();
  studentId = loginCred.get('id');
  const payload = await (async () => {
  const key = await HASH_SHA256(new Date().toISOString() + ':' + crypto.getRandomValues(new Uint32Array(1))[0]);
  ...
  return {
    ...
    formData: {
      id: loginCred.get('id'),
      hash: await HASH_SHA256(
        (await HASH_SHA256(loginCred.get('pass'))) + key
      )
    }
  };
})();
        ...
        try {
            ...
            document.getElementById('currentDateTime').style.display = 'flex';
            loadTimezone();
            testData = data;
            sessionStorage.setItem(await HASH_SHA256(studentId), await HASH_SHA256(data.loginCode))
            document.getElementById('loginSection').innerHTML = `<h1 class='text-primary'>${data.nameguard?data.nameguard:"Error Loading Student Name"}</h1>`;
            createWeekSchedule('scheduleBox',convertTo2D(data.batches,4,","));
      ...
      const response = await fetch(loginUrl, {
        ...
        body: JSON.stringify({
          ...
          formData: {
            id: studentId,
            batchName: batchName,
            loginCode: sessionStorage.getItem(await HASH_SHA256(studentId))
          }
        })
      });
...
dateList.innerHTML =  `<div class="pad2 bold block round2 mar3 pw80 text-center mar2 lmar shadow2">Select the following green (available) dates, just by clicking</div>`;
